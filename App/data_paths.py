...
def render_data_path_error(logs: list[str]) -> str:
    ...
    lines = [
        ...
        "",
        "Fix options (production — do not upload client CSVs to App Service):",
        "1. Enable PostgreSQL — USE_POSTGRES=true + POSTGRES_* App Settings; run",
        "   init_db.py + migrate_to_postgres.py from a trusted local machine (CSV stays local).",
        "2. Local dev only — keep CSVs under Data/Revenue_Tracker_Migration/ (gitignored).",
        "3. Optional override — SLAM_DATA_PATH only for non-production diagnostics.",
        "",
        "Path diagnostics:",
    ]
