...
**Version**: 2.38  
**Date**: May 24, 2026  
**Status**: **Future bank statement & accounting automation roadmap documented** — new subsection 8.1 captures the heavy-OCR Azure Function path, automated P&L generation, persistent rules engine, intelligent check-image linking, OneDrive auto-sync, and broader modern accounting features tailored to SLAM Services. Documentation-only update; `APP_VERSION` unchanged. **Cursor is primary agent**; Kilo Code secondary.

## Change Log

- **v2.38 (May 24, 2026)**: Documented future bank statement & accounting automation roadmap. Added new subsection 8.1 "Future Bank Statement & Accounting Automation" with Azure Function OCR path, P&L generation, persistent rules engine, check-image linking, OneDrive integration, and expanded accounting features. `APP_VERSION` not changed in code (documentation-only update).
...
This subsection captures the forward-looking roadmap for evolving the bank statement pipeline and overall accounting workflow into a fuller, modern bookkeeping platform tailored to SLAM Services. These items extend (and do not replace) the current lightweight in-app pipeline (`Scripts/bank-statement-parser.py` + Bank Statements page) and remain compatible with the existing CSV / Power Query workflow.

- **Dedicated Azure Function for heavy OCR processing**: Offload `OpenCV`, `EasyOCR`, `pdf2image`, and `Tesseract` workloads from the Streamlit App Service to a purpose-built Azure Function (or Container App). The main app uploads the PDF, the Function returns structured transactions, cropped check images, and confidence metadata. Keeps the App Service slim, fast, and within reasonable Azure SKU limits while enabling true scanned-PDF support without bloating the deploy package.
...
- **QuickBooks-style persistent rules engine**: A learned mapping layer that records each (description → payee → category/account) decision Laura or Stef makes and applies it to future statements. Includes fuzzy payee matching (handles `WALMART #1234`, `WAL-MART STORE`, `WM SUPERCENTER`, etc.), per-client overrides, and confidence-aware suggestions that improve over time.
...
- **Broader modern accounting features tailored to SLAM Services**: Role-based views (Laura / Stef / Patty / Robert), client-facing invoicing, receivables tracking with aging buckets, an enhanced audit trail (who changed what, when, and why), and client-specific dashboards that surface only the data relevant to each business. Together these move the platform from a revenue-chasing tool toward a defensible, multi-user bookkeeping practice system.

These items are explicitly future / aspirational and will be sequenced under Phase 3 of the Phased Roadmap (Section 10). No code changes are required to adopt this subsection — it documents intent and direction only.
