...
**Version**: 2.27  
**Date**: May 24, 2026  
**Status**: **Local dev stabilized** — clean Python 3.10 `.venv` recreated (`Setup-LocalVenv.ps1`); Streamlit + imports verified locally; Azure live post-v2.26 manual zip deploy confirmed. **P0 Azure CSV path fix** (v2.26) in production. **Phase 3 foundation** — `App/db_utils.py`, `Scripts/migrate_to_postgres.py`, dual-mode `USE_POSTGRES`. Phase 2.5 **P0–P2 complete**. **Cursor is primary agent**; Kilo Code secondary.

## Change Log

- **v2.27 (May 24, 2026)**: Local runtime stabilization after v2.26 dependency changes. Recreated `.venv` with Python 3.10.11; resolved WinError 5 pip failures caused by orphaned Streamlit/Python processes locking `.venv`. Added `Scripts/PowerShell/Setup-LocalVenv.ps1` for repeatable venv rebuild. Verified: `numpy`/`pandas`/`streamlit`/`sqlalchemy` imports, `ruff check App/ Scripts/`, migration `--dry-run` (99 clients / 36 requests), local `streamlit run App/app.py` (HTTP 200). Azure live site confirmed post-v2.26 manual zip deploy. **Remaining follow-ups**: PostgreSQL provisioning + `USE_POSTGRES` enablement; PostgreSQL write-back from Revenue Requests editor; push v2.26 commit to `origin/main`.
