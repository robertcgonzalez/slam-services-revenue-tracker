...
**Version**: 2.39  
**Date**: May 25, 2026  
**Status**: **Lightweight persistent payee rules engine is live (Section 8.1 Quick Parallel Win — Delivered).** Bank Statements now auto-cleans Grok Vision + parser output via a QuickBooks-style mapping table in `Data/payee_rules.csv` (gitignored, seeded with 25 common merchant patterns). Rules apply automatically after every parser run and Grok CSV paste, preserve Laura's manual edits, and grow over time through a **💡 Learn this mapping** form on the review page. Modern polling-safe Azure deploy path (v2.38.3) remains the standard. **Cursor is primary agent**; Kilo Code secondary.
