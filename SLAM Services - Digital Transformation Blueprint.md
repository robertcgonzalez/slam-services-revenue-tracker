...
**Deploy note (v2.25):** P0–P2 fixes ship via `main` + GitHub Actions (`.github/workflows/deploy-azure.yml`) once `AZUREAPPSERVICEPUBLISHPROFILE` is set, or via manual flat zip (`az webapp deployment source config-zip`) per v2.10. Client CSVs remain on App Service / Kudu — not in git.
