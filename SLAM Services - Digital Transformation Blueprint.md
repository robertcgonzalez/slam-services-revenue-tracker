...
**Version**: 2.26  
**Date**: May 24, 2026  
**Status**: **P0 Azure CSV path fix** — robust `resolve_data_path()` in `App/data_paths.py`, deployment hardening (`clean: false` in GitHub Actions, `Build-AzureDeployZip.ps1`, startup diagnostics). **Phase 3 foundation** — `App/db_utils.py`, `Scripts/migrate_to_postgres.py`, dual-mode `USE_POSTGRES` in `App/app.py` (CSV fallback). Phase 2.5 **P0–P2 app stabilization complete** in `App/app.py`; **GitHub Actions deploy** requires `AZUREAPPSERVICEPUBLISHPROFILE` secret; manual zip deploy with `Data/` documented. **Agent git practices** (v2.24). **Cursor is the primary / lead AI coding agent**; Kilo Code secondary.

## Change Log

- **v2.26 (May 24, 2026)**: P0 Azure data loading fix + Phase 3 DB prep. `App/data_paths.py` — expanded path candidates, `SLAM_DATA_PATH` override, wwwroot diagnostics, clear error messaging. `App/app.py` — dual mode `USE_POSTGRES` with CSV fallback; PostgreSQL loaders via `App/db_utils.py`. `Scripts/migrate_to_postgres.py` — CSV → PostgreSQL migration skeleton (Blueprint Section 7 schema). Deployment: `startup.sh` lists Data folder; `.github/workflows/deploy-azure.yml` uses flat `deploy-package/` with `clean: false` (preserves App Service `Data/`); `Scripts/PowerShell/Build-AzureDeployZip.ps1` for manual flat zip including local `Data/`. `requirements.txt` adds SQLAlchemy + psycopg2-binary. README deploy + verification steps updated.
...
**Deploy note (v2.26):** GitHub Actions deploys **code only** (`clean: false` preserves existing `Data/` on App Service). First-time or full data deploy: use `Scripts/PowerShell/Build-AzureDeployZip.ps1` locally (includes `Data/`) or upload `Data/Revenue_Tracker_Migration/` via Kudu. Optional App Setting: `SLAM_DATA_PATH=/home/site/wwwroot/Data/Revenue_Tracker_Migration`. Phase 3: `USE_POSTGRES=true` after `Scripts/migrate_to_postgres.py` and Azure PostgreSQL provisioning.
