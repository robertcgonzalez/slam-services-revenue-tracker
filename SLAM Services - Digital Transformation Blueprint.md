...
**Version**: 2.20  
**Date**: May 24, 2026  
**Status**: Phase 2.5 P0 complete. **Cursor Edition** environment from v2.14 retained (`.vscode/` workspace, Ruff/Black, Streamlit/Azure tasks). **Python 3.10 local/Azure parity** (v2.17): `.venv` recreated with Python 3.10.11 to match `runtime.txt`. **Azure CLI 64-bit** (v2.18): migrated from 32-bit install to official x64 build (v2.86.0) for better crypto/performance. **`App/app.py` Ruff-clean** (v2.19): import order, formatting, and `zip(..., strict=True)` verified — zero Ruff issues. **Workspace cleanup** (v2.20): removed unused Continue.dev `.continue/` artifacts; `.gitignore` updated. **Cursor is the primary / lead AI coding agent**; Kilo Code remains available as secondary. Rules updated in `.cursor/rules/slam-services.mdc`, `.kilocode`, and README.
